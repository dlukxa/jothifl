...
class Playlist:
    ...
    def create_genre_playlist(self, taste):
        genre_playlist = Playlist()

        current = self.head
        while current:
            if current.taste == taste:
                genre_playlist.add_song(current.song_name, current.artist, current.taste)
            current = current.next

        if genre_playlist.head is None:
            print(f"No songs found in the playlist with the genre '{taste}'.")
        else:
            print(f"Created a playlist with songs of the genre '{taste}':")
            genre_playlist.display_playlist()
        self.head = genre_playlist.head
        return self.head
    ...
    def display_playlist(self):
        if self.head is None:
            ...
            while current:
                print(f"{path_num}. '{current.song_name}' by {current.artist}")
                current = current.next
                path_num += 1
...
while inp != "q":
    ...
    if inp == 'd' or inp == 'D': 
        ...
        # Create a new playlist with songs of the taste
        print("🎵 JothiFl ~ Tell me your taste ?")
        inp = input("🎵 JothiFl ~ ")
        playlist.create_genre_playlist(inp.capitalize())
        ...
        # Playing song from the playlist with number
        playlist.play_song_by_path_num(int(inp))

    elif inp == 'help':
        print("Displaying the playlist             : d/D")
        print("Removing a song from the playlist   : dele")
        print("Playing the current song            : p/P")
        print("Playing the next song               : n/N")
        print("Playing the previous song           : r/R")
        print("Shuffling the playlist              : s/S")
        print("Create a new playlist with songs of the taste : taste")
